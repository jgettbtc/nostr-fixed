import {
  ...
  import { dispatchNostrMessage } from "./nostr-dispatch.js";
  ...
  export const nostrPlugin: ChannelPlugin<ResolvedNostrAccount> = {
    ...
    meta: {
      id: "nostr-fixed",
      label: "Nostr Fixed",
      selectionLabel: "Nostr Fixed",
      blurb: "Decentralized DMs via Nostr relays (NIP-04) - Dev",
      order: 100,
    },
    capabilities: {
    ...
    reload: { configPrefixes: ["channels.nostr-fixed"] },
    configSchema: buildChannelConfigSchema(NostrConfigSchema),
  
    config: {
      listAccountIds: (cfg) => listNostrAccountIds(cfg, "nostr-fixed"),
      resolveAccount: (cfg, accountId) => resolveNostrAccount({ cfg, accountId, channelKey: "nostr-fixed" }),
      defaultAccountId: (cfg) => resolveDefaultNostrAccountId(cfg, "nostr-fixed"),
    ...
    security: {
      resolveDmPolicy: ({ account }) => {
        return {
          ...
          policyPath: "channels.nostr-fixed.dmPolicy",
          allowFromPath: "channels.nostr-fixed.allowFrom",
          approveHint: formatPairingApproveHint("nostr-fixed"),
          normalizeEntry: (raw) => {
            try {
    ...
    outbound: {
      ...
      sendText: async ({ to, text, accountId }) => {
        ...
        return { channel: "nostr-fixed", to: normalizedTo };
      },
    },
